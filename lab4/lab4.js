...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadReviews() {
        const savedReviews = CookieManager.load();
    ...
    function renderReviews() {
        ...
        reviews.forEach(review => {
            ...
            if (review.image && review.image.trim() !== '') {
                imageHtml = `<img src="${review.image}" class="review-image" onerror="this.style.display='none'">`;
            }
    ...
    function validateForm(name, text) {
        ...
        if (name.length > 30) return 'Имя слишком длинное.';
        if (!text) return 'Текст отзыва не может быть пустым.';
        if (text.length > 500) return 'Отзыв слишком длинный.';
        return '';
    }
